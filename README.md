...
| **Vitest + coverage** | 단위 테스트 통과 + 임계 (lines 60% / statements 60% / functions 60% / branches 50%) |

### Coverage 정책

- **임계**: lines/statements/functions ≥ 60%, branches ≥ 50% (PR merge 차단 기준)
- **Scope** (vitest.config.ts coverage.exclude 정의):
  - 포함: 06-entities/model · 06-entities/api · 05-features/model · 07-shared/errors · 07-shared/api/base · app/providers
  - 제외: UI 컴포넌트(.tsx), Next.js framework 파일(page/layout/error), Supabase 클라이언트, Sprint 2 placeholder
- **로컬 확인**: `npm run test:coverage` → HTML 리포트 `coverage/index.html`
- **PR artifact**: GitHub Actions가 coverage 리포트를 14일 보관 (PR Actions 페이지에서 다운로드)
- **점진 상향 정책**: Sprint 2 baseline 60%. Sprint 3+ 70/75/80% 단계적 상향 검토
...
npm run test:coverage # 단위 테스트 + 커버리지 임계 확인
npm run build         # 빌드 확인
```
